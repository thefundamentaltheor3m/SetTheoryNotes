...
\begin{boxdefinition}[Transitive Set]
    % [CORRECTED] was: $\forall y \in A \brac{y \subseteq A \lr \forall y \in A \, \forall x \in y \, \parenth{x \in A}}$ --- the inner $\forall y$ rebinds $y$, so the bracket reads ``$y \subseteq A$ iff $A$ is transitive'' and is satisfied by non-transitive sets such as $\set{\set{\emptyset}}$; the two formulations were meant as equivalent restatements, not as a biconditional inside the quantifier
    A set $A$ is \textbf{transitive} if $\forall y \in A \, \parenth{y \subseteq A}$, ie, if $\forall y \in A \, \forall x \in y \, \parenth{x \in A}$
\end{boxdefinition}
...
\subsection{Mostowski Collapse for Well-Orderings}

We will eventually prove this in more generality (\Cref{Ch2:Thm:Mostowski}), but here's a useful theorem.

\begin{boxtheorem}[Mostowski Collapse Theorem for Well-Orderings]\label{Ch1:Thm:Mostowski_WO}
