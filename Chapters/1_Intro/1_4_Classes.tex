...
% [CORRECTED] was: ``for every nonempty $S \ssq A$'' --- $S$ has to be a \textit{set}. As written it reads as quantifying over subclasses, which bites wherever $A$ is a proper class: it stops being a single formula of the language of set theory and becomes a scheme, and it makes chapter 2's first proposition --- billed there as extending well-foundedness from subsets to subclasses --- a restatement of the definition, with its set-likeness hypothesis and its whole proof idle. Where $A$ is a set variable, as in ``$\forall A \in \WF$, $(A, \in)$ is well-founded'' and in the TFAE below, the two readings agree by Comprehension, so nothing in this section was affected
