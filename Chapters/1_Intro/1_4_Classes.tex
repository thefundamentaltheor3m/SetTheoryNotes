...
% [CORRECTED] was: ``for every nonempty $S \ssq A$'' --- $S$ has to be a \textit{set}. As written it reads as quantifying over subclasses, and then it is not a formula of the language of set theory at all but a scheme, which the uses below need it not to be: both ``$\forall A \in \WF$, $(A, \in)$ is well-founded'' and the TFAE bind $A$, so ``well-founded'' must be a formula with $A$ free. It also makes chapter 2's first proposition --- billed there as extending well-foundedness from subsets to subclasses --- a restatement of the definition, with its set-likeness hypothesis and its whole proof idle
