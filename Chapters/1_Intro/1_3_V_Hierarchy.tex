...
\begin{enumerate}
    ...
    % [CORRECTED] was: $y\of{z \cup \set{z}} = \Powset\of{z}$ --- the recursion has to take the power set of the previous \textit{value}, $y\of{z} = V_z$, not of the index $z$; with $\Powset\of{z}$ the sequence agrees with the $V_n$ only up to $n = 3$ (by the coincidences $V_0 = 0$, $V_1 = 1$, $V_2 = 2$) and gives $8$ elements rather than $16$ at $n = 4$
    \item If $z \cup \set{z} \in \dom{y}$, then $z \in \dom{y}$ and $y\of{z \cup \set{z}} = \Powset\of{y\of{z}}$.
\end{enumerate}
The idea is that $y = \cycl{V_0, V_1, V_2, \ldots, V_{n}}$\footnote{By this, we mean it is a \textit{finite sequence}, ie, a function from $n + 1$ to something that takes $0$ to $V_{0}$, $1$ to $V_{1}$, and so on up to $n$.} for some $n \in \omega$. Then,
\begin{align*}
...
\begin{align*}
    \grpres{V_n}{n < \omega}
\end{align*}
% [FILLED] answers the author's own ``% OF WHAT?'' at this line; the lecture said only ``by taking a union''
by taking the union of its range. Each $\grpres{V_m}{m \leq n}$ is a function with domain $n + 1$, ie, a set of ordered pairs, and each one is contained in the next, so the union of all of them is a function too: the one with domain $\bigcup_{n < \omega} \parenth{n + 1} = \omega$ that sends $n$ to $V_n$.
