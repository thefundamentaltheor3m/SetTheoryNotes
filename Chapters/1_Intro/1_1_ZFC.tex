...
% [CORRECTED] was: ``with $E \subseteq \parenth{A \times A}$ representing equality'' --- the language has one relation symbol, $\in$, and $E$ is its interpretation; equality is logical and is not a component of the structure, which is also what the Completeness step just below needs
Before proceeding further, we define the empty set. In first order logic, structures are required to have non-empty universes. Thus, for every structure $\parenth{A, E}$ in the language of set theory, with $E \subseteq \parenth{A \times A}$ representing membership, we must have
\begin{align*}
...
\begin{align*}
    \bigcup \calF = \setst{x}{\exists A \in \calF \, \parenth{x \in A}}
\end{align*}
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[dot/.style = {circle, fill, inner sep = 1.5pt}, every node/.style = {font = \small}]
        \node[draw, thick, ellipse, minimum width = 1.8cm, minimum height = 3.2cm, label = {above:$A$}] at (0, 0) {};
        \node[draw, thick, ellipse, minimum width = 2.2cm, minimum height = 4.2cm, label = {above:$B$}] at (4.5, 0) {};

        \node[dot, label = {left:$x_0$}] (x0) at (0, 0.9) {};
        \node[dot, label = {left:$x_1$}] (x1) at (0, 0.15) {};
        \node[dot, label = {left:$x_2$}] (x2) at (0, -0.6) {};
        \node at (0, -1.15) {$\vdots$};

        \node[dot, label = {right:$y_0$}] (y0) at (4.5, 1.35) {};
        \node[dot, label = {right:$y_1$}] (y1) at (4.5, 0.6) {};
        \node[dot, label = {right:$y_2$}] (y2) at (4.5, -0.15) {};
        \node at (4.5, -0.7) {$\vdots$};

        \draw[->] (x0) -- node[above, sloped] {$\varphi$} (y0);
        \draw[->] (x1) -- (y1);
        \draw[->] (x2) -- (y2);
    \end{tikzpicture}
\end{figure}

$B$ is drawn with room to spare because the axiom only asks for \textit{some} set containing the witnesses; cutting it down to exactly them is what \Cref{ZFC:Compr} does.
...
\begin{align*}
    f = \setst{\parenth{x, y} \in A \times B}{\varphi\of{x, y, \bar{z}}}
\end{align*}
