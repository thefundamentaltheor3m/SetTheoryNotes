...
% [CORRECTED] was: $\Defin(A) := \setst{x}{x \text{ is a 1st-order definable structure over } (A; \in) \text{ using parameters from } A}$ --- the operation that builds $L$ collects the definable \textit{subsets} of $A$; without $x \ssq A$ neither the agreement with $\Powset$ on finite sets nor $L_n = V_n$ holds (a definable binary relation on $\set{\emptyset}$ is already not a subset of it)
...
\begin{align*}
    \Defin(A) &:= \setst{x \ssq A}{x \text{ is 1st-order definable over } (A; \in) \text{ using parameters from } A}
\end{align*}
...
\begin{boxnotation}
    For a function $f$, we write $\dom{f}$ for its domain.
\end{boxnotation}
