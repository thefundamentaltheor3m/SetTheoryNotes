...
% [CORRECTED] was: ``given a cardinal $\kappa$'' --- for finite $\kappa = n$ every well-ordering of $n$ has type $n$, so the supremum below is $n$ rather than $n + 1$; the construction gives the least cardinal above $\kappa$ exactly when $\kappa$ is infinite
Given AC, we know that $2^{\kappa} > \kappa$ and we can define $\kappa^{+}$ to be the least cardinal strictly greater than $\kappa$. Without AC, we still know, given an infinite cardinal $\kappa$, that
\begin{align*}
...
% [IGNORE] Author's instruction (10 Sep 2026): leave this \sorry as it is. /fill-sorries, /post-lecture and every other skill skip it; it stays until the author closes it by hand.
It's clear \sorry

With the $\beth$s in hand, we can say exactly how big every level of the hierarchy is, from $V_{\omega}$ onwards.
...
The proposition indexes the levels from $V_{\omega}$ rather than from $V_0$, but the shift by $\omega$ is invisible from $\omega^2$ onwards.
