...
Before starting, rename the bound variables of $\varphi$ so that no variable occurring in $\psi$ is bound in $\varphi$: if $u$ occurs in $\psi$ and $\varphi$ binds $u$, replace $\varphi$ by $\varphi\of{u / u'}$ for some fresh $u'$. Write $\varphi'$ for the result, so that $A = \setst{x}{\varphi'(x)}$ still, and use $\varphi'$ in place of $\varphi$ throughout. The reason is the quantifier step below, which substitutes a variable $u$ of $\psi$ for the free variable $x$ of $\varphi$: were $u$ bound in $\varphi$, that substitution would capture it, and $\varphi\of{u}$ would say something other than ``$u \in A$''.
\begin{itemize}
...
\begin{proof}
    ...
    \end{align*}
    Apply \Cref{Ch2:Lemma:Relativisation_Preserves_Proofs} with $\sigma$ the conjunction $\sigma_1 \land \cdots \land \sigma_n \land F$ and $\tau$ the contradiction. Relativisation commutes with the propositional connectives, by the propositional step of the definition, so what comes out is
    \begin{align*}
