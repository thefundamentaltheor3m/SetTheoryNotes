...
\begin{boxexample}[Sequences of Ordinals]
    ...
    % [CORRECTED] the clause fixing the length convention for $<_{\lex}$ is inserted; the text stated none, and the other reading (a proper initial segment counts as \textit{larger}) is false here --- it makes $\emptyset$ the maximum, so the predecessors of $\emptyset$ are a proper class, set-likeness fails, and the collapse cannot land on $(\OR, <)$, which has no largest element
