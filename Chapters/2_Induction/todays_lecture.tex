...
\begin{proof}
    ...
    % [CORRECTED] $\exists x \parenth{x \in \WF}$ is added to the left of the display, which had only the relativised axioms --- the lemma's non-emptiness hypothesis is not logically valid and the relativised axioms do not prove it, so it has to be carried here and discharged in the paragraph below, where we are working in $\ZFwoF$ and $\emptyset \in \WF$ is available
    \begin{align*}
